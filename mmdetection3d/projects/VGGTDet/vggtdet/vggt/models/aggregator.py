...
from projects.VGGTDet.vggtdet.vggt.layers import PatchEmbed
from projects.VGGTDet.vggtdet.vggt.layers.block import Block
from projects.VGGTDet.vggtdet.vggt.models.vggt import RotaryPositionEmbedding2D, PositionGetter
from projects.VGGTDet.vggtdet.vggt.models.vggt import vit_small, vit_base, vit_large, vit_giant2
...
class Aggregator(nn.Module):
    ...
    def forward(
        ...
        if vis_attn_map or if_use_atten_sample:
            ...
            #     img_patch_attn = images_patch_attn[img_idx]
            #     attn_img = img_patch_attn.reshape(H//self.patch_size, W//self.patch_size) #.cpu().numpy()
                
            #     if isinstance(images_bak[0][img_idx], torch.Tensor):
            #         img = images_bak[0][img_idx].permute(1,2,0).cpu().numpy()
            #     else:
            #         img = images_bak[0][img_idx]
                
            #                 size=(img.shape[0], img.shape[1]),  # (H, W)
            #                 mode='bicubic',
                # attn_img_up = cv2.resize(attn_img, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_CUBIC)

                # ######## yang #########
                # min_val = torch.min(attn_img_up)
                # max_val = torch.max(attn_img_up)

                # if max_val == min_val:
                #     norm_attn_img_up = torch.ones_like(attn_img_up)
                # else:
                #     norm_attn_img_up = (attn_img_up - min_val) / (max_val - min_val)
                # # norm_attn_img_up = (attn_img_up - torch.min(attn_img_up)) / (torch.max(attn_img_up) - torch.min(attn_img_up))
                # num_point = prob_dist_pre.shape[-1]
                # sampled_indices = torch.multinomial(prob_dist, num_samples, replacement=False)
                # sampled_value = prob_dist_pre[sampled_indices]


                # mask_flat = torch.zeros_like(prob_dist)

                # mask = mask_flat.view(*norm_attn_img_up.shape)

                # sampled_coords = torch.stack((
                #                                 torch.div(sampled_indices, 
                #                                         norm_attn_img_up.size(1), 
                #                             ), dim=-1)

                # attn_img_up = attn_img_up * mask
                # attn_img_up = attn_img_up.cpu().numpy()
                # print(1)
                # ######## yang #########


                # axes[row, 0].imshow(img)
                # axes[row, 0].set_title(f'Image {img_idx+1} (Original)')
                # axes[row, 0].axis('on')
                
                # axes[row, 1].imshow(attn_img_up, cmap='jet')
                # axes[row, 1].set_title(f'Image {img_idx+1} (Attention)')
                # axes[row, 1].axis('on')
                
                # axes[row, 2].imshow(img)
                # axes[row, 2].imshow(attn_img_up, cmap='jet', alpha=0.3)
                # axes[row, 2].set_title(f'Image {img_idx+1} (Overlay)')
                # axes[row, 2].axis('on')
            
                # plt.tight_layout()
                # plt.savefig('im2_attention_grid_4x3_filtered_v3.png', bbox_inches='tight', pad_inches=0)
